  ...
  return (
    <div className="w-full max-w-6xl mx-auto py-12 px-4">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {plans.map((plan) => {
          const bgClass = plan.id === 'free' ? 'bg-[#6C2F83]/10 dark:bg-[#6C2F83]/20' :
                          plan.id === 'payg' ? 'bg-[#181E53]/10 dark:bg-[#181E53]/20' :
                          plan.id === 'basic' ? 'bg-[#502D81]/10 dark:bg-[#502D81]/20' :
                          plan.id === 'plus' ? 'bg-[#3C38A4]/10 dark:bg-[#3C38A4]/20' :
                          plan.id === 'premium' ? 'bg-[#6C2F83]/10 dark:bg-[#6C2F83]/20' :
                          'bg-[#181E53]/10 dark:bg-[#181E53]/20';
          const borderClass = plan.id === 'free' ? 'border-[#6C2F83]/20' :
                              plan.id === 'payg' ? 'border-[#181E53]/20' :
                              plan.id === 'basic' ? 'border-[#502D81]/20' :
                              plan.id === 'plus' ? 'border-[#3C38A4]/20' :
                              plan.id === 'premium' ? 'border-[#6C2F83]/20' :
                              'border-[#181E53]/20';
          const buttonClass = plan.id === 'free' ? 'bg-[#6C2F83] hover:bg-[#502D81]' :
                              plan.id === 'payg' ? 'bg-[#181E53] hover:bg-[#3C38A4]' :
                              plan.id === 'basic' ? 'bg-[#502D81] hover:bg-[#6C2F83]' :
                              plan.id === 'plus' ? 'bg-[#3C38A4] hover:bg-[#181E53]' :
                              plan.id === 'premium' ? 'bg-[#6C2F83] hover:bg-[#502D81]' :
                              'bg-[#181E53] hover:bg-[#3C38A4]';
          return (
            <div key={plan.id} className={`${bgClass} rounded-xl shadow-lg overflow-hidden transition-transform hover:scale-[1.02] hover:shadow-xl border-2 ${borderClass}`}>
              <div className="p-6">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-xl font-bold mb-2 text-white">{plan.name}</h3>
                    <div className="text-3xl font-bold mb-2 text-white">₹{plan.price}<span className="text-sm text-white font-normal">/month</span></div>
                  </div>
                  {plan.id === 'plus' && (
                    <span className={`${buttonClass} text-white text-xs font-semibold px-3 py-1 rounded-full`}>Most Popular</span>
                  )}
                </div>
                ...
                {plan.id === 'enterprise' ? (
                  <button
                    className={`w-full py-3 px-4 ${buttonClass} text-white font-medium rounded-lg transition`}
                    onClick={() => {
                  ...
                  user ? (
                    <button
                      className={`w-full py-3 px-4 bg-gray-300 dark:bg-gray-700 text-white font-medium rounded-lg cursor-not-allowed`}
                      disabled
                    >
                  ...
                  <RazorpayHandler
                    ...
                    customClassName={`w-full py-3 px-4 ${buttonClass} text-white font-medium rounded-lg transition-all focus:outline-none focus:ring-2 focus:ring-opacity-50`}
                  />
                ) : (
                  <button
                    className={`w-full py-3 px-4 ${buttonClass} text-white font-medium rounded-lg transition-all focus:outline-none focus:ring-2 focus:ring-opacity-50`}
                    onClick={handleLoginPrompt}
                  >
                    Get {plan.name} Plan
                  </button>
