...
const festivalGreetings: { [key: string]: string } = {
  ...
  "Valentine's Day": "Happy Valentine'ts Day",
...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          details:
            "Brand name, industry, logo, and festival are all required.",
    ...
    const logoBase64 = `data:${logo.type};base64,${Buffer.from(
      logoBuffer
    ).toString("base64")}`;
    ...
    const imageWithTextUrl = await addTextOverlay(
      baseImageUrl,
      festival,
      customTitle,
      customSubtitle,
    ...
    const finalImageUrl = await overlayLogoUsingEditAPI(
      imageWithTextUrl,
      logoBase64,
      baseUrl
    );
...
async function generateBaseImage(prompt: string, baseUrl: string): Promise<string> {
  try {
    ...
    throw new Error(
      `Base image generation failed: ${error instanceof Error ? error.message : "Unknown error"}`
    );
  }
}

async function overlayLogoUsingEditAPI(
  imageUrl: string,
  logoBase64: string,
  baseUrl: string
): Promise<string> {
  try {
    ...
    if (imageUrl.startsWith("data:image/")) {
      ...
      if (!imageResponse.ok) {
        throw new Error(
          `Failed to fetch image for logo overlay: ${imageResponse.status} ${imageResponse.statusText}`
        );
      }
    ...
    // Since edit API now returns data URL directly, return it without uploading to R2
...
async function addTextOverlay(
  ...
  try {
    ...
    if (imageUrl.startsWith("data:image/")) {
      ...
      if (!imageResponse.ok) {
        throw new Error(
          `Failed to fetch image for text overlay: ${imageResponse.status} ${imageResponse.statusText}`
        );
      }
      const imageBuffer = await imageResponse.arrayBuffer();
      base64Image = Buffer.from(imageBuffer).toString("base64");
    ...
    if (!customSubtitle) {
      ...
      try {
        ...
        if (subtitleResponse.ok) {
          ...
          if (
            finalSubtitle.length > 10 &&
            finalSubtitle.length <= 100
          ) {
            ...
            const hasFestiveContent = festiveWords.some((word) =>
              finalSubtitle.toLowerCase().includes(word)
            );
...
Make sure both lines of text are clearly visible and readable against the background. Use high contrast colors (white text with dark outline/shadow if needed). Position the text in the upper portion with appropriate spacing between the two lines. Do not change anything else in the image - keep all existing content exactly as it is.`;
    ...
    // Return data URL directly instead of uploading to R2
    const base64 = generatedBuffer.toString("base64");
...
async function generateFestivePrompt(
  brand: string,
  industry: string,
  festival: string,
  logoColors?: string
